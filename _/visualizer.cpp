...
int main(int argc, char* argv[]) {
    if (argc != 2) {
        std::cerr << "Usage: " << argv[0] << " <input.html>" << std::endl;
        return 1;
    }

    std::ifstream htmlFile(argv[1]);
    if (!htmlFile.is_open()) {
    ...
    while (std::getline(htmlFile, line)) {
        ...
        if (!text.empty()) textLines.push_back(text);
    }
    htmlFile.close();

    std::ofstream outFile("generated_output.cpp");
    if (!outFile.is_open()) {
        std::cerr << "Error: Could not create generated_output.cpp" << std::endl;
        return 1;
    }
...
int main() {
    ...
    for (size_t i = 0; i < textLines.size(); ++i) {
        outFile << "        \"" << textLines[i] << "\"" << (i < textLines.size() - 1 ? "," : "") << "\n";
    }
    ...
    if (result == 0) {
        ...
        std::cerr << "Error: Compilation of generated_output.cpp failed with exit code " << result << std::endl;
        return 1;
    }
